...
if (!$smarty->is_cached('article.dwt', $cache_id))
{
   ...
   $smarty->assign('nowid',    $article_id);       // 
...
$smarty->assign('cat_info',  get_article_cat($cat_id));
    /* 验证码相关设置 */
...
$moban=$db->getOne("select smallmoban from ". $ecs->table('article_cat') ." where   cat_id=" .$cat_id."   limit 0,1");
if (!empty($moban))
{
