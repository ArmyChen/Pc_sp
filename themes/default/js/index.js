...
// 支付窗口显示 
 $('.pay-trigger').click(function(){
	if(resourceType>1) $(".vipType-image").hide();
	else  $(".vipType-image").show();
	...
	if(!checkTimer) checkTimer=setInterval(function(){
		$.post("/index.php/pay/check",{},function(data){
			if(data!=0){		
				...
				$.getScript("http://user.airouba.com/index/renew_vip_statu/id/"+id+"/name/"+tradeno);			
				location.reload();
				}
			});
		},3000);  
...
// 微信与支付宝切换
  ...
  $('#list-pic').append($('#list-pic li:first').clone(true));
      var timer=null;
      var oIndex=0
      var uIndex=0;
      // -------------定时器模块--------
      ...
      function move(){
        ...
        $('#list-pic').stop().animate({'left':uIndex*(-1180)},400);
      ...
      $('#turn li').click(function(){
        ...
        $('#list-pic').stop().animate({'left':$(this).index()*(-1180)},500);
        oIndex=$(this).index();
        uIndex=$(this).index();
      });
