...
/**
 * 取得车系列表
 * @return array 车系列表 id => name   阿能
 */
function get_chexi_list()
{
    $sql = 'SELECT chexi_id, chexi_name FROM ' . $GLOBALS['ecs']->table('chexi') . ' ORDER BY sort_order';
    $res = $GLOBALS['db']->getAll($sql);

    $chexi_list = array();
    foreach ($res AS $row)
    {
        $chexi_list[$row['chexi_id']] = addslashes($row['chexi_name']);
    }
...
function get_chexis($cat = 0, $app = 'chexi')
{
    global $page_libs;
    ...
    $sql = "SELECT b.chexi_id, b.chexi_name, b.chexi_logo, b.chexi_desc, COUNT(*) AS goods_num, IF(b.chexi_logo > '', '1', '0') AS tag 
".
            "FROM " . $GLOBALS['ecs']->table('chexi') . "AS b, ".
                $GLOBALS['ecs']->table('goods') . " AS g ".
            "WHERE g.chexi_id = b.chexi_id $children AND is_show = 1 " .
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
            "GROUP BY b.chexi_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC";
    if (isset($static_page_libs[$template]['/library/chexis.lbi']))
    {
        $num = get_library_number("chexis");
        $sql .= " LIMIT $num ";
    }
    ...
    {
        $row[$key]['url'] = build_uri($app, array('cid' => $cat, 'cheid' => $val['chexi_id']), $val['chexi_name']);
        $row[$key]['chexi_desc'] = htmlspecialchars($val['chexi_desc'],ENT_QUOTES);
    }
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
            ...
            " AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
    "GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY tag DESC, b.sort_order ASC LIMIT 0,5"; //阿能开发，省略
...
case 'suppliers':
            ...
            else
            {
                ...
                {
                    /* 代码修改_start  By www.ecshop120.com */
					$define_url_suppliers=$GLOBALS['db']->getOne("select define_url from ". $GLOBALS['ecs']->table
("suppliers") ." where suppliers_id='$supid' limit 0,1");
                    $uri = $define_url_suppliers ? 's-'. trim($define_url_suppliers) : 's-' . $supid;

					if (isset($chdid) && !empty($cheid))
                    {
                        $uri .= '-che' . $cheid;
					}
...
['cat_id'].'/';
        ...
        case 'brand':
            ...
            else
            {
                ...
                {
                    /* 代码修改_start  By www.ecshop120.com */
					$define_url_brand=$GLOBALS['db']->getOne("select define_url from ". $GLOBALS['ecs']->table
("brand") ." where brand_id='$bid' limit 0,1");
...
$cat_array['cat_id'].'/';
     ...
     if ($rewrite)
    {
        ...
        /* 代码修改_start By www.ecshop120.com */
        if($app == 'category' or $app == 'brand' or $app == 'article_cat' or $app == 'suppliers')
		{
			$uri .= '/';
		}
		else
