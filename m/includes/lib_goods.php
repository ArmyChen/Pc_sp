...
function index_tag_key_par($cat_aid)
{

$sql = "SELECT * FROM " . $GLOBALS['ecs']->table('category') . " where parent_id = ".$cat_aid."   ";
$res = $GLOBALS['db']->getAll($sql);
$arr = array();
foreach ($res AS $idx => $row)
    {
		...
		$arr[$idx]['cat_name']          = $row['cat_name'];
		      $arr[$idx]['catinfosmlist']          = index_tag_key_par_list($row['cat_id']);
    }
...
function index_tag_key_par_list($cat_aid)
{

 $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('content_key') . " where cat_id = ".$cat_aid." and is_com=1 and is_show=1   ";
 $res = $GLOBALS['db']->getAll($sql);
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.click_url, g.nick, g.market_price, g.shop_price AS org_price, g.promote_price, ' .//lyf改
...
function get_promote_goods($cats = '')
{
 ...
 $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    ...
    foreach ($result AS $idx => $row)
    {
	 ...
	 $goods[$idx]['selled_count']   = selled_count($row['goods_id']);
		$goods[$idx]['comments_number']   = $row['comments_number'];
		        $goods[$idx]['tuijie_img']   = $row['tuijie_img'];
...
$row['saving']    = $market_price - $promote_price;
	$row['saving2']    = $market_price - $shop_price;
	$row['save_rate'] = $market_price ? round(($promote_price/ $market_price), 2)*10 : 0;
$row['save_rate2'] = $market_price ? round(($shop_price/ $market_price), 2)*10 : 0;
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
                "g.goods_attr_id, g.attr_value, g.imgurl, g.img_thumb, g.attr_price " . //lyf改
    ...
    foreach ($res AS $row)
    {
        ...
        else
        {
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
														...
														'imgurl'        => $row['imgurl'], //lyf改
														'img_thumb'        => $row['img_thumb'], //lyf改
...
function get_goods_gallery($goods_id)
{
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc' .
        ...
        " WHERE goods_id = '$goods_id' ORDER BY img_id ASC LIMIT " . $GLOBALS['_CFG']['goods_gallery_number']; //lyf改
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price,g.nick, g.shop_price AS org_price, ' .//lyf改
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
 ...
 * @return  array
 */
function get_promote_goods_nocom($cats = '')
{
    $time = gmtime();
    $order_type = $GLOBALS['_CFG']['recommend_order'];
 ...
 $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and is_best=0 AND g.is_delete = 0 ' .
            " AND g.is_promote = 1 AND promote_start_date <= ".$time." AND promote_end_date >= ".$time." ";
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT 12 ";
    $result = $GLOBALS['db']->getAll($sql);
...
function get_goods_com($cats = '')
{
 ...
 $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and is_best=1  AND g.is_delete = 0 ' .
            "   ";
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT 12 ";
    $result = $GLOBALS['db']->getAll($sql);
...
function get_goods_hot($cats = '')
{
 ...
 $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, g.is_new, g.is_best, g.is_hot, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, " .
                "g.is_best, g.is_new, g.is_hot, g.is_promote, RAND() AS rnd " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and is_hot=1  AND g.is_delete = 0 ' .
            "   ";
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT 12 ";
    $result = $GLOBALS['db']->getAll($sql);
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['name']         = $row['goods_name'];
        $goods[$idx]['brief']        = $row['goods_brief'];
    $goods[$idx]['fang_url'] = price_format($row['fang_url']);
...
$goods[$idx]['thumb']       = isset($demogoodss['thumb'][$row['goods_id']])?$demogoodss['thumb'][$row['goods_id']]:get_image_path($row['goods_id'], $row['thumb'], true);
//载入demo数据
...
function get_goods_new($cats = '')
{
 ...
 $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and is_new=1  AND g.is_delete = 0 ' .
            "   ";
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT 12 ";
    $result = $GLOBALS['db']->getAll($sql);
...
function get_sg_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.comments_number, g.sales_volume,g.market_price, g.shop_price AS org_price, g.promote_price,g.tuijie_img, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 or g.is_alone_sale = 1 and g.is_best=1 AND g.is_delete = 0 ' .
            " or g.is_promote = 1  ";
    $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';
    $sql .= " LIMIT $num";
    $result = $GLOBALS['db']->getAll($sql);
    ...
    $goods[$idx]['is_new']   = $row['is_new'];
	  $goods[$idx]['is_hot']   = $row['is_hot'];
...

        {
			...
			$goods[$idx]['gmt_end_time'] =  $promote_end_date;
        }
        else
        {
            $goods[$idx]['gmt_end_time'] = 0;
