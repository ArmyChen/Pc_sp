...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('ur_here',     $_LANG['mad_list_pc']);
    $smarty->assign('action_link', array('text' => $_LANG['mad_list_pc_add'], 'href' => 'ads_pc.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('ads_list_pc.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('ads_list_pc.htm'), '',
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',       $_LANG['mad_list_pc_add']);
    $smarty->assign('action_link',   array('href' => 'ads_pc.php?act=list', 'text' => $_LANG['mad_list_pc']));
     $smarty->assign('position_list', get_position_list_pc());
    ...
    assign_query_info();
    $smarty->display('ads_info_pc.htm');
}
...
elseif ($_REQUEST['act'] == 'insert')
{
 ...
 /*if ((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] == 0) || (!isset($_FILES['ad_img_en']['error']) 
&& isset($_FILES['ad_img_en']['tmp_name'] ) &&$_FILES['ad_img_en']['tmp_name'] != 'none'))
        {
            $ad_code_en = basename($image->upload_image($_FILES['ad_img_en'], 'afficheimg'));
        }
        if (!empty($_POST['img_url_en']))
        {
            $ad_code_en = $_POST['img_url_en'];
        }
        if (((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] > 0) || (!isset($_FILES['ad_img_en']
['error']) && isset($_FILES['ad_img_en']['tmp_name']) && $_FILES['ad_img_en']['tmp_name'] == 'none')) && empty($_POST
['img_url_en']))
        {
            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
            sys_msg($_LANG['js_languages']['ad_photo_empty'], 0, $link);
    ...
    /* 插入数据 */
    $sql = "INSERT INTO ".$ecs->table('ad'). " (position_id,media_type,ad_name,ad_link,ad_code,start_time,end_time,link_man,cat_id,pcat_id,link_email,link_phone,click_count,enabled)
    ...
    $link[1]['href'] = 'ads_pc.php?act=list';

    $link[2]['text'] = $_LANG['continue_add_ad'];
    $link[2]['href'] = 'ads_pc.php?act=add';
...
/*if ((isset($_FILES['ad_img_en']['error']) && $_FILES['ad_img_en']['error'] == 0) || (!isset($_FILES['ad_img_en']['error']) && isset($_FILES['ad_img_en']['tmp_name']) && $_FILES['ad_img_en']['tmp_name'] != 'none'))
        {
            $en_img_up_info = basename($image->upload_image($_FILES['ad_img_en'], 'afficheimg'));
            $ad_code_en = "ad_code_en = '".$en_img_up_info."'".',';
        }
        else
        {
            $ad_code_en = '';
   ...
   /* 提示信息 */
   $href[] = array('text' => $_LANG['back_ads_list'], 'href' => 'ads_pc.php?act=list');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $url = 'ads_pc.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
