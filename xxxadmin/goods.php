...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    /* 模板赋值 */
    $goods_ur = array('' => $_LANG['mgoods_list'], 'virtual_card'=>$_LANG['50_virtual_card_list']);
    $ur_here = ($_REQUEST['act'] == 'list') ? $goods_ur[$code] : $_LANG['mgoods_trash'];
    $smarty->assign('ur_here', $ur_here);

    $action_link = ($_REQUEST['act'] == 'list') ? add_link($code) : array('href' => 'goods.php?act=list', 'text' => $_LANG['mgoods_list']);
...
/* 如果是安全模式，检查目录是否存在 */
    ...
    /* 模板赋值 */
    $smarty->assign('code',    $code);
    $smarty->assign('ur_here', $is_add ? (empty($code) ? $_LANG['mgoods_add'] : $_LANG['51_virtual_card_add']) : ($_REQUEST['act'] == 'edit' ? $_LANG['edit_goods'] : $_LANG['copy_goods']));
...
$fang_url = empty($_POST['fang_url']) ? '' : $_POST['fang_url'];
$down_url_dx = empty($_POST['down_url_dx']) ? '' : $_POST['down_url_dx'];
$down_url_lt = empty($_POST['down_url_lt']) ? '' : $_POST['down_url_lt'];
    /* 入库 */
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,fencheng, suppliers_id, tuijie_img,fang_url,down_url_dx,down_url_lt)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$fencheng', '$suppliers_id', '$tuijie_img', '$fang_url', '$down_url_dx', '$down_url_lt')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,fencheng, tuijie_img,fang_url,down_url_dx,down_url_lt)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[goods_spcdesc]','$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$fencheng', '$tuijie_img', '$fang_url', '$down_url_dx', '$down_url_lt')";
        }
    }
    else
    {
...
if($tuijie_img)
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
			...
			"goods_spcdesc = '$_POST[goods_spcdesc]', " .
				...
				"fencheng = '$fencheng', " .
			"fang_url = '$fang_url', " .
			"down_url_dx = '$down_url_dx', " .
			"down_url_lt = '$down_url_lt', " .
                "is_best = '$is_best', " .
                "is_new = '$is_new', " .
                "is_hot = '$is_hot', " .
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    {
        $link[] = array('href' => 'goods.php?act=trash', 'text' => $_LANG['mgoods_trash']);
    }
    else
    {
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    if (empty($goods))
    {
        $link[] = array('href' => 'goods.php?act=list', 'text' => $_LANG['mgoods_list']);
        sys_msg($_LANG['cannot_found_goods'], 1, $link);
    }
    ...
    $smarty->assign('action_link',  array('href' => 'goods.php?act=list', 'text' => $_LANG['mgoods_list']));
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    if ($insert)
    {
         $link[] = array('href' => 'goods.php?act=add', 'text' => $_LANG['mgoods_add']);
         $link[] = array('href' => 'goods.php?act=list', 'text' => $_LANG['mgoods_list']);
         $link[] = array('href' => 'goods.php?act=product_list&goods_id=' . $product['goods_id'], 'text' => $_LANG['18_product_list']);
    }
    else
    {
         $link[] = array('href' => 'goods.php?act=list&uselastfilter=1', 'text' => $_LANG['mgoods_list']);
...
function list_link($is_add = true, $extension_code = '')
{
    ...
    {
        $text = $GLOBALS['_LANG']['mgoods_list'];
    }
